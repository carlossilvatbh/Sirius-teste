...
from .models import Entity, Structure, EntityOwnership, StructureNode, NodeOwnership, Party
...
class PartyAdmin(admin.ModelAdmin):
    """Admin para Pessoas e Sócios"""
    list_display = ['name', 'party_type', 'tax_identification_number', 'nationality', 'get_structures_count']
    list_filter = ['party_type', 'nationality', 'created_at']
    search_fields = ['name', 'tax_identification_number', 'email']
    readonly_fields = ['created_at', 'updated_at', 'get_structures_count']
    
    fieldsets = (
        ('👤 Informações Pessoais', {
            'fields': ('name', 'party_type', 'nationality')
        }),
        ('📋 Documentação', {
            'fields': ('tax_identification_number', 'passport_number', 'email')
        }),
        ('📊 Estatísticas', {
            'fields': ('get_structures_count', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
    
    def get_structures_count(self, obj):
        count = obj.nodeownership_set.values('owned_node__structure').distinct().count()
        return f"{count} estrutura(s)"
    get_structures_count.short_description = 'Participação em Estruturas'
